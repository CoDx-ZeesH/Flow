...
class _ActiveSessionScreenState extends State<ActiveSessionScreen>
  ...
  // ✅ FIX 1: Timer now starts at 0 when the session begins
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                _buildRhythmSegment(context, isTrough: true), const SizedBox(width: 6),
                _buildRhythmSegment(context, isPeak: true), const SizedBox(width: 6),
                _buildRhythmSegment(context, isPeak: true), const SizedBox(width: 6),
                _buildRhythmSegment(context, isCurrent: true), const SizedBox(width: 6),
                _buildRhythmSegment(context, isUpcoming: true), const SizedBox(width: 6),
  ...
  // ✅ FIX 3 & 4: Uniform sizes and elegant breathing animation for the active box
  Widget _buildRhythmSegment(BuildContext context, {bool isTrough = false, bool isPeak = false, bool isCurrent = false, bool isUpcoming = false}) {
    ...
    // Exact same dimensions for every block
    Widget box = Container(
      width: 28, height: 32, 
      decoration: BoxDecoration(
        color: isCurrent ? bgColor : bgColor.withValues(alpha: isUpcoming ? 0.2 : 0.6),
    ...
    if (isCurrent) {
      // Connects to your existing _blinkController to create a breathing opacity effect
      return FadeTransition(
        opacity: Tween<double>(begin: 0.35, end: 1.0).animate(CurvedAnimation(parent: _blinkController, curve: Curves.easeInOutSine)),
        child: box,
      );
    }
    return box;
